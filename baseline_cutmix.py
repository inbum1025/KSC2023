...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, inputs, noise, transforms, alpha):
        self.inputs = inputs
        self.noise = noise
        self.transforms = transforms
        self.alpha = alpha

    def mix_up(self, input, noise):
        mixed_img = self.alpha * input + (1 - self.alpha) * noise

        return mixed_img

    def rand_bbox(self, size, alpha):
        W = size[1]
        H = size[2]
        cut_rat = np.sqrt(1. - alpha)
    ...
    def cut_mix_up(self, input, noise):
        bbx1, bby1, bbx2, bby2 = self.rand_bbox(input.size(), self.alpha)

        input[:, bbx1:bbx2, bby1:bby2] = noise[:, bbx1:bbx2, bby1:bby2]
...
class Datamodule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        ...
        self.train_sub_dataset = Dataset(train_inputs, train_inputs, train_transform, self.alpha)
        ...
        self.test_dataset = Dataset(train_inputs, test_inputs, test_transform, self.alpha)
        self.test_sub_dataset = Dataset(train_inputs, val_inputs, test_transform, self.alpha)
...
def cut_mix_up(input, noise):
    bbx1, bby1, bbx2, bby2 = rand_bbox(input.size(), alpha)

    input[:, bbx1:bbx2, bby1:bby2] = noise[:, bbx1:bbx2, bby1:bby2]

    return input

def make_positive(anchor, noise):
    anchor2 = anchor.clone().detach()
    positive = cut_mix_up(anchor2, noise)

    return positive

def mix_up(input, noise):
...
if __name__ == '__main__':
    ...
    model_list = glob.glob('/workspace/ksc2023/model/UT-R50*.ckpt')
    print()
    for model_path in ['/workspace/ksc2023/model/UT-R50-vien-tri-0.9_mixup-epoch=11-val_loss=0.02.ckpt']:
        # model = Model.load_from_checkpoint('/workspace/ksc2023/model/UT-R50-vien-tri-0.9_cutmix-epoch=19-val_loss=0.01.ckpt', model_name=model_name, data_name=data_name)
        model = Model.load_from_checkpoint(model_path, model_name=model_name, data_name=data_name)

    # logger = pl.loggers.WandbLogger(project='ksc_brand', name=f"{model_type}_{data_name}_cutmix_{alpha}")

    # trainer = pl.Trainer(accelerator='gpu', devices=[args.gpu], max_epochs=max_epochs, callbacks=[pl.callbacks.ModelCheckpoint(dirpath='model', monitor='val_loss', filename=f"{model_type}-{data_name}-{alpha}"+'_cutmix-{epoch:02d}-{val_loss:.2f}', save_top_k=1), pl.callbacks.EarlyStopping(monitor="val_loss", mode="min", patience=5)], logger=logger, log_every_n_steps=1)
    # trainer.fit(model=model, datamodule=datamodule)
    # trainer.test(model=model, datamodule=datamodule)
    #     model.evaluate(datamodule.train_dataloader_no_shuffle(), datamodule.test_dataloader())
    # model.visualize(datamodule.test_dataloader(), f'html/cutmix/cutmix-{alpha}')
        ...
        img_1 = transform(Image.open('data/psim_1.png')).to(model.device)
        img_2 = transform(Image.open('data/psim_2.jpg')).to(model.device)

        al, ai = model.model(img_1.unsqueeze(0))
        pl, pi = model.model(img_2.unsqueeze(0))

        print(model_path)
        print(pdist(al, pl))

        img_1 = transform(Image.open('data/sim_1.jpg')).to(model.device)
        # img_2 = transform(Image.open('data/sim_2.png')).to(model.device)
        img_2 = transform(Image.open('data/lsim_1.png')).to(model.device)
        # img_2 = mix_up(img_1, img_2)
